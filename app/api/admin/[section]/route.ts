...
if (ownerOnly(section) && profile?.role !== "owner") {
  return NextResponse.json(
    ...
    if (table === "stylists") {
      ...
      const allowedFields = new Set(["name", "bio", "category", "active"]);
    ...
    const allowedFieldsByTable: Record<string, Set<string>> = {
      services: new Set(["name", "category", "description", "price", "duration_minutes", "active"]),
      working_hours: new Set(["stylist_id", "day_of_week", "start_time", "end_time"]),
      blocked_periods: new Set(["stylist_id", "start_time", "end_time", "reason"]),
      coupons: new Set(["code", "discount_type", "discount_value", "minimum_amount", "usage_limit", "expires_at", "active", "event_name", "source", "reason"]),
    };
    const allowedFields = allowedFieldsByTable[table];
    if (!allowedFields) return NextResponse.json({ error: "Unsupported operation." }, { status: 400 });
    const clean = Object.fromEntries(
      Object.entries(body).filter(([key]) => allowedFields.has(key))
    );
    ...
    console.error("Admin section create failed", e);
    return NextResponse.json({ error: "Unable to complete the requested operation." }, { status: 500 });
  }
}
