    ...
    const customerIds = Array.from(
      new Set(
        rows
          .map(
            (coupon) =>
              coupon.compensation_customer_id
          )
          .filter(Boolean)
      )
    );
    ...
    const appointmentIds = Array.from(
      new Set(
        rows
          .map(
            (coupon) =>
              coupon.compensation_appointment_id
          )
          .filter(Boolean)
      )
    );
    ...
    const compensationCoupons =
      rows.map((coupon) => {
        ...
        const whatsappMessage =
          `Hi ${customerName}, we're really sorry for the inconvenience caused by the cancellation of your appointment with us. We'd like to offer you ${discount}% off your next appointment as an apology.\n\nYour coupon code is: ${coupon.code}\n\nThis coupon can be used once and is valid until ${expiryText}.\n\nWe hope to see you again soon. ❤️`;
