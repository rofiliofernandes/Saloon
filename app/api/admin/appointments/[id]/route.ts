...
function buildWhatsAppMessage(
  ...
  return `Hi ${firstName}, we're very sorry for the inconvenience caused by the cancellation of your appointment with ${stylistName} at AK Hair & Beauty Salon.
...
async function buildCompensationResponse(
  ...
  const { data: sourceAppointment } = await s
    .from("appointments")
    .select("stylist_id")
    .eq("id", (coupon as any).source_appointment_id || "")
    .maybeSingle();

  const { data: stylist } = sourceAppointment?.stylist_id
    ? await s
        .from("stylists")
        .select("name")
        .eq("id", sourceAppointment.stylist_id)
        .maybeSingle()
    : { data: null };
