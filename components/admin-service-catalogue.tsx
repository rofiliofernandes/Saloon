...
function openEditService(service: Service) {
  ...
  setServiceCategory(service.category_id || "");
  setAudiences(
    (service.service_audiences || []).map(
      (item) => item.audience
    )
  );
  setOptions(
    (service.service_options || []).map((option) => ({
      ...
      duration_minutes: String(option.duration_minutes),
    }))
  );
  setServiceModal(true);
}
...
async function saveService(
  ...
  /*
   * A variation name is optional when there is
   ...
   * 90 min
   *
   * Internally the option will be called "Standard".
   *
   * If there are multiple options, each one should
   * have a useful name such as:
   * Short / Medium / Long
  ...
  if (options.length > 1) {
    for (const [index, option] of options.entries()) {
      if (!option.name.trim()) {
        setError(
          `Please give pricing option ${index + 1} a variation name.`
        );
        return;
      }
    }
  ...
  for (const [index, option] of options.entries()) {
    if (
      ...
      setError(
        `Enter a valid price for pricing option ${index + 1}.`
      );
      return;
    }
    ...
    if (
      ...
      setError(
        `Enter a valid duration for pricing option ${index + 1}.`
      );
      return;
    }
  }
  ...
  /*
   * Give a single unnamed option a sensible internal
   * name so the database still receives a valid value.
   */
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40 p-5">
          <div className="w-full max-w-md rounded-3xl bg-white p-6 shadow-2xl">
            ...
            <p className="mt-4 text-sm leading-6 text-neutral-500">
              This removes the service from the active catalogue. Existing appointments remain safe.
            </p>
            ...
            <div className="mt-6 flex gap-3">
              <button
                ...
                onClick={() => setDeletingService(null)}
                disabled={deleting}
                className="flex-1 rounded-xl border border-black/10 px-4 py-3 text-sm font-medium transition hover:bg-neutral-50 disabled:opacity-50"
              >
                Cancel
              </button>
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    const response = await fetch(
                      `/api/admin/service-catalogue?id=${deletingService.id}`,
                      { method: "DELETE" }
                    );
                    ...
                    setError(
                      e?.message || "Unable to delete service."
                    );
                    setDeletingService(null);
                  } finally {
                    setDeleting(false);
